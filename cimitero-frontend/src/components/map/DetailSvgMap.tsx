// src/components/map/DetailSvgMap.tsx
"use client";
...
export default function DetailSvgMap({ src, className = "", selectedPointId }: Props) {
    ...
    function screenToSvgCoords(clientX: number, clientY: number) {
        ...
        return {
            x: vb.x + (localX - offsetX) / scale,
            y: vb.y + (localY - offsetY) / scale
        };
    }
    ...
    useEffect(() => {
        ...
        const targetEl = pathsRef.current.find((p) => String(p.id) === String(selectedPointId));

        if (targetEl) {
            pathsRef.current.forEach(p => styleDimmed(p));
            styleSelected(targetEl);
        }
    ...
    useEffect(() => {
        ...
        function handlePointerMove(e: PointerEvent) {
            ...
            setViewBox(clampViewBox({
                x: startViewBox.x - svgDx,
                y: startViewBox.y - svgDy,
    ...
    return (
        <div className={`relative h-full w-full ${className}`}>
            ...
            <div className="absolute right-4 top-4 z-20 flex flex-col gap-2">
                ...
                <button type="button" onClick={() => originalViewBoxRef.current && setViewBox(originalViewBoxRef.current)} className="rounded-lg border border-gray-200 bg-white/95 px-3 py-2 text-xs font-bold uppercase tracking-widest text-gray-700 shadow hover:bg-white">Reset</button>
            </div>
        </div>
    );
}